<?php
	...
	$baza = new Baza();
	/* definicija upita */
	$upit = "SELECT objava.objava_id, naslov_objave, opis_objave, date_format( datum_objavljivanja_objave,'%d.%m.%Y') as ' datum_objavljivanja_objave', korisnik.korisnik_id, ime_prezime, slika_korisnika_krug, url_slike FROM objava INNER JOIN korisnik ON objava.korisnik_id = korisnik.korisnik_id INNER JOIN fotografije_objava ON objava.objava_id = fotografije_objava.objava_id WHERE fotografija_objava_id % 2 = 0;";
	...
	while($red = $rezultat->fetch_array()){
		...
		$myObj->opisObjave = json_decode($red["opis_objave"]);
